  ...
  _addDomainChecks(url, findings, features) {
    try {
      ...
      try {
        const parsedUrl = new URL(url);
        const hostname = parsedUrl.hostname;
        // Pattern: starts with 2+ random letters, then 2+ digits, then .tld (e.g., xz05116.cn)
        if (
          /^[a-z]{2,}\d{2,}\.[a-z]{2,}$/.test(hostname) ||
          /^[a-z]{2,}\d{2,}\.[a-z0-9.-]+$/.test(hostname)
        ) {
          findings.push({
            text: 'Suspicious domain name pattern',
            description: 'This domain uses a pattern of random letters followed by numbers, which is common for automatically generated malicious domains.',
            severity: 'high',
            category: 'domain'
          });
        }
        // No vowels in domain (excluding tld)
        const domainPart = hostname.split('.')[0];
        if (domainPart && !/[aeiou]/i.test(domainPart)) {
          findings.push({
            text: 'Domain contains no vowels',
            description: 'Legitimate domains almost always contain vowels. Domains without vowels are often generated by bots for malicious purposes.',
            severity: 'high',
            category: 'domain'
          });
        }
      ...
      // === WHOIS INFORMATION ISSUES ===
      try {
        // If creation date is missing or zero, but expiration date exists and is > 0
        if (
          ('time_domain_activation' in features && Number(features.time_domain_activation) === 0) &&
          ('time_domain_expiration' in features && Number(features.time_domain_expiration) > 0)
        ) {
          findings.push({
            text: 'Domain creation date unavailable',
            description: 'This domain is missing creation date information in its WHOIS record. This is unusual for legitimate domains and is typical of newly created or intentionally hidden phishing domains.',
            severity: 'medium',
            category: 'domain'
          });
        }
